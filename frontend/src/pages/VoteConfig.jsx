...
const BACKDROP_PRESETS = [
  ...
  { key: "odc",    label: "ODC Orange",  css: "linear-gradient(135deg, #DE6A00 0%, #1e1b4b 100%)" },
    ...
    <div>
      <div className="flex items-center justify-between mb-5">
        <div>
          ...
          <p className="text-xs text-slate-500 mt-0.5">Pour le public — pas de vote, pronostic uniquement</p>
        </div>
      ...
      <div className="flex flex-col items-center gap-4">
        {qrDataUrl ? (
          ...
          >
            ...
            <div className="absolute inset-0 rounded-2xl bg-black/0 group-hover:bg-black/5 transition-colors flex items-center justify-center">
              <Maximize2 className="w-6 h-6 text-slate-500 opacity-0 group-hover:opacity-100 transition-opacity" />
            </div>
          </button>
        ) : (
        ...
        <div className="w-full flex items-center gap-2 rounded-xl border border-slate-200 bg-slate-50 px-3 py-2.5">
          ...
          <button onClick={copyLink} title="Copier le lien" className="flex-shrink-0 text-slate-500 hover:text-orange-500 transition-colors">
  ...
  return (
    ...
    <div className="max-w-3xl">
        ...
        {editInfo ? (
          <div className="space-y-3">
            ...
            <div>
              ...
              <p className="text-xs text-slate-500 mt-1">Timer affiché au jury pendant le pitch</p>
            </div>
            <div>
              ...
              <p className="text-xs text-slate-500 mt-1">Timer Q&amp;R lancé manuellement par l'admin</p>
            </div>
                ...
                {BACKDROP_PRESETS.map(p => {
                  ...
                  return (
                    ...
                    >
                      ...
                      />
                      <span className="text-xs text-slate-500 leading-none">{p.label}</span>
                    </button>
                  );
                })}
              </div>
              ...
              {(() => {
                ...
                return isFile ? (
                  <div className="mt-2 flex items-center gap-2 rounded-xl border border-green-200 bg-green-50 px-3 py-2.5">
                    ...
                    <button type="button" onClick={() => setInfoForm(f => ({ ...f, backdrop: "" }))} className="text-slate-500 hover:text-red-500 flex-shrink-0"><X className="w-3.5 h-3.5" /></button>
                  </div>
                ) : (
          ...
          <div className="text-sm text-slate-700 space-y-1">
            ...
            {session.backdrop && (
              <p className="flex items-center gap-2">
                ...
                <span className="text-slate-500 text-xs truncate max-w-[200px]">{BACKDROP_PRESETS.find(b => b.css === session.backdrop)?.label || "Image personnalisée"}</span>
              </p>
            )}
          </div>
        )}
      ...
      <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm mb-4">
        <div className="flex items-center justify-between mb-4">
          <h2 className="font-semibold text-slate-800">
            Projets <span className="text-slate-500 font-normal text-sm">({session.projects?.length || 0})</span>
          </h2>
          {session.status !== "closed" && (
            <button
            ...
            <div key={p.id} className="flex items-start gap-3 rounded-xl border border-slate-100 bg-slate-50 px-4 py-3">
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 flex-wrap">
                  ...
                  {p.is_female_led && (
                    <span className="text-xs font-semibold bg-pink-100 text-pink-700 rounded-full px-2 py-0.5">♀ Portée par une femme</span>
                  )}
                </div>
              ...
              <div className="flex items-center gap-1 flex-shrink-0">
                ...
                {session.status !== "closed" && (
                  <>
                    ...
                    <button onClick={() => startEditProject(p)} className="p-1.5 rounded-lg hover:bg-slate-200 text-slate-500 hover:text-slate-700 transition-colors"><Edit2 className="w-3.5 h-3.5" /></button>
                    <button onClick={() => deleteProject(p.id)} className="p-1.5 rounded-lg hover:bg-red-100 text-slate-500 hover:text-red-500 transition-colors"><Trash2 className="w-3.5 h-3.5" /></button>
                  </>
                )}
              </div>
            </div>
            ...
            <div>
              ...
              {presType === "url" ? (
                ...
                <div className="mt-2 flex items-center gap-2 rounded-xl border border-green-200 bg-green-50 px-3 py-2.5">
                  ...
                  <button type="button" onClick={() => setProjForm(p => ({ ...p, presentation_pdf: "" }))} className="text-slate-500 hover:text-red-500 flex-shrink-0">
                ...
                <label className={`mt-2 flex items-center gap-2 rounded-xl border border-dashed border-slate-300 px-3 py-4 cursor-pointer hover:border-orange-400 hover:bg-orange-50 transition-colors ${uploadingPdf ? "opacity-50 pointer-events-none" : ""}`}>
                  ...
                  {uploadingPdf
                    ...
                    : <><Upload className="w-4 h-4 text-slate-500" /><span className="text-xs text-slate-500">PDF (recommandé, max 200 Mo) ou PPTX (max 25 Mo — limite Office Online)</span></>
                  }
                </label>
              )}
              <p className="text-xs text-slate-500 mt-0.5">Pour les PPTX volumineux, préférer Google Slides via le champ &quot;Lien&quot;</p>
            </div>

            {/* Vidéo */}
            <div>
              <label className="text-xs font-medium text-slate-600">Vidéo (optionnel)</label>
              <div className="mt-1 grid grid-cols-2 gap-1.5">
                <button
                  type="button"
                  onClick={() => { setVideoType("url"); setProjForm(p => ({ ...p, video_file: "" })); }}
                  className={`rounded-xl border px-3 py-2 text-xs font-medium transition-colors flex items-center justify-center gap-1.5 ${videoType === "url" ? "border-orange-400 bg-orange-50 text-orange-700" : "border-slate-200 text-slate-600 hover:bg-slate-50"}`}
                >
                  <ExternalLink className="w-3 h-3" /> Lien URL
                </button>
                <button
                  type="button"
                  onClick={() => { setVideoType("file"); setProjForm(p => ({ ...p, video_url: "" })); }}
                  className={`rounded-xl border px-3 py-2 text-xs font-medium transition-colors flex items-center justify-center gap-1.5 ${videoType === "file" ? "border-orange-400 bg-orange-50 text-orange-700" : "border-slate-200 text-slate-600 hover:bg-slate-50"}`}
                >
                  <Upload className="w-3 h-3" /> Fichier vidéo
                </button>
              </div>
              {videoType === "url" ? (
                <input
                  ...
                  value={projForm.video_url}
                  onChange={e => setProjForm(p => ({ ...p, video_url: e.target.value }))}
                />
              ) : projForm.video_file ? (
                <div className="mt-2 flex items-center gap-2 rounded-xl border border-green-200 bg-green-50 px-3 py-2.5">
                  <span className="text-xs text-green-700 flex-1 truncate font-mono">🎬 {projForm.video_file}</span>
                  <button type="button" onClick={() => setProjForm(p => ({ ...p, video_file: "" }))} className="text-slate-500 hover:text-red-500 flex-shrink-0">
                    <X className="w-3.5 h-3.5" />
                  </button>
                </div>
              ) : (
                <label className={`mt-2 flex items-center gap-2 rounded-xl border border-dashed border-slate-300 px-3 py-4 cursor-pointer hover:border-orange-400 hover:bg-orange-50 transition-colors ${uploadingVideo ? "opacity-50 pointer-events-none" : ""}`}>
                  ...
                  {uploadingVideo
                    ...
                    : <><Upload className="w-4 h-4 text-slate-500" /><span className="text-xs text-slate-500">MP4, WebM, MOV — max 150 Mo (préférer YouTube pour les grandes vidéos)</span></>
                  }
                </label>
              )}
            </div>
      ...
      <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm mb-4">
        <div className="flex items-center justify-between mb-4">
          <h2 className="font-semibold text-slate-800">
            Critères de notation <span className="text-slate-500 font-normal text-sm">({session.criteria?.length || 0})</span>
          </h2>
          ...
          {session.criteria?.map((c) => (
            <div key={c.id} className="flex items-center gap-3 rounded-xl border border-slate-100 bg-slate-50 px-4 py-3">
              <div className="flex-1 min-w-0">
                ...
                {c.description && <p className="text-xs text-slate-500 mt-0.5 truncate italic">{c.description}</p>}
              </div>
              {session.status === "draft" && (
                <div className="flex gap-1">
                  <button onClick={() => startEditCrit(c)} className="p-1.5 rounded-lg hover:bg-slate-200 text-slate-500 hover:text-slate-700 transition-colors"><Edit2 className="w-3.5 h-3.5" /></button>
                  <button onClick={() => deleteCriterion(c.id)} className="p-1.5 rounded-lg hover:bg-red-100 text-slate-500 hover:text-red-500 transition-colors"><Trash2 className="w-3.5 h-3.5" /></button>
                </div>
              )}
            </div>
          ))}
            ...
            <div className="flex flex-col items-center gap-4">
              ...
              <div className="w-full flex items-center gap-2 rounded-xl border border-slate-200 bg-slate-50 px-3 py-2.5">
                ...
                <button
                  ...
                  className="flex-shrink-0 text-slate-500 hover:text-orange-500 transition-colors"
                >
                  {copied
