...
const STATUS_BADGE = {
  ...
  closed: { label: "Termin̩",    cls: "bg-green-100 text-green-700" },
};
...
export default function Vote() {
  ...
  const handleDelete = async (id) => {
    if (!confirm("Supprimer cette session et toutes ses donn̩es ?")) return;
    try {
    ...
    <div>
      ...
      {showNew && (
        <form onSubmit={handleCreate} className="mb-6 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              <label className="text-xs font-medium text-slate-600">Nom de l&apos;̩v̩nement *</label>
              <input
                required
            ...
            <div>
              <label className="text-xs font-medium text-slate-600">Date de l&apos;̩v̩nement</label>
              <input
                type="date"
                className="mt-1 w-full rounded-xl border border-slate-200 px-3 py-2.5 text-sm focus:border-orange-400 focus:outline-none focus:ring-2 focus:ring-orange-100"
          ...
          <div className="flex gap-3 mt-4">
            ...
            >
              {saving ? "Cr̩ation..." : "Cr̩er et configurer"}
            </button>
            <button
              type="button"
      ...
      {loading ? (
        ...
        <div className="text-center py-16 text-slate-400">
          ...
          <p className="text-sm mt-1">Cr̩ez une session pour commencer</p>
        </div>
      ) : (
              ...
              <div key={s.id} className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm flex items-center gap-4">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-4 mt-1 text-xs text-slate-500 flex-wrap">
                    ...
                    <span className="flex items-center gap-1">
                      ...
                      {s.jury_count} jur̩{s.jury_count !== 1 ? "s" : ""}
                    </span>
                  </div>
                </div>
                <div className="flex items-center gap-2 flex-shrink-0">
                  ...
                  {s.status === "active" && (
                    ...
                    >
                      <Play className="w-3.5 h-3.5" /> G̩rer
                    </button>
                  )}
                  {s.status === "closed" && (
                    <button
