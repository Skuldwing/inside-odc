    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="card p-6">
          <h3 className="font-semibold text-slate-900 mb-3">Répartition par plateforme</h3>
          <div className="h-72">
            {loading ? (
              <div className="h-full flex items-center justify-center text-slate-400">Chargement...</div>
          ...
          {loading ? (
            ...
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={monthly}>
                ...
                <Tooltip />
                <Legend />
                <Line type="monotone" dataKey="reach" stroke="#F97316" strokeWidth={2} name="Portée" />
                <Line type="monotone" dataKey="engagement" stroke="#10B981" strokeWidth={2} name="Engagement" />
                <Line type="monotone" dataKey="results" stroke="#3B82F6" strokeWidth={2} name="Résultats" />
              </LineChart>
            </ResponsiveContainer>
          )}
        </div>
  ...
  return (
    <div className="card p-4">
      ...
      <p className={`text-xs mt-1 ${positive ? "text-green-600" : "text-red-600"}`}>
        {positive ? "+" : ""}
        {Number(growth || 0)}% vs période précédente
      </p>
    </div>
  );
}
