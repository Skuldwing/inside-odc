...
import {
  Plus, FileText, ArrowLeft, Layers, Users, CheckSquare, ListChecks, X,
} from "lucide-react";
import api from "../api";
...
export default function FormulaireEditor() {
  ...
  const addField = (type = "text", page = null) => {
    ...
    const maxPage = editor.fields.length > 0
      ? Math.max(...editor.fields.map(f => Number(f.page || 1)))
      : 1;
    const targetPage = page ?? maxPage;
    setEditor(prev => ({ ...prev, fields: [...prev.fields, { ...createField(type, idx), page: targetPage }] }));
  ...
  const addPage = () => {
    ...
    setEditor(prev => ({
      ...prev,
      fields: [...prev.fields, { ...createField("text", idx), page: newPage }],
    }));
    setCollapsedFields(prev => ({ ...prev, [editor.fields.length]: false }));
  };
  ...
  const duplicateField = (index) =>
    setEditor(prev => {
      ...
      const copy  = { ...field, _id: newId, key: field.type === "separator" ? `sep_${prev.fields.length + 1}` : `${field.key}_copie`, label: `${field.label} (copie)` };
              ...
              <div className="space-y-3">
                ...
                {(() => {
                  ...
                  return pageNums.map(pageNum => {
                    const pageFields = editor.fields
                      .map((field, idx) => ({ field, idx }))
                      .filter(({ field }) => Number(field.page || 1) === pageNum);
                    ...
                    return (
                      <div key={pageNum} className="space-y-2">
                          ...
                          <div className="flex items-center gap-2 pt-1">
                            ...
                            <div className="flex items-center gap-1">
                              <span className="inline-flex items-center gap-1.5 rounded-full border border-orange-200 bg-orange-50 px-3 py-1 text-xs font-semibold text-orange-700">
                                ...
                                Page {pageNum}
                                <span className="text-orange-400 font-normal">· {pageFields.length} champ{pageFields.length !== 1 ? "s" : ""}</span>
                              </span>
                              {pageNum > 1 && (
                                <button
                                  type="button"
                        ...
                        {/* + Ajouter un champ sur cette page */}
                        <button
                          type="button"
                          className="w-full rounded-xl border-2 border-dashed border-slate-200 py-2.5 text-xs text-slate-400 hover:border-orange-300 hover:text-orange-500 transition-colors"
                          onClick={() => addField("text", pageNum)}
                        >
                          <Plus className="w-3.5 h-3.5 inline mr-1" />
                          {multiPage ? `Ajouter un champ – Page ${pageNum}` : "Ajouter un champ texte"}
                        </button>
                      </div>
                    );
                  });
                ...
                <button
                  type="button"
                  className="w-full rounded-xl border-2 border-dashed border-orange-200 py-3 text-sm font-medium text-orange-500 hover:border-orange-400 hover:bg-orange-50 transition-colors flex items-center justify-center gap-2"
                  onClick={addPage}
                >
