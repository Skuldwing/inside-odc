  ...
  try {
    const result = await pool.query(
      `SELECT id, name, type, message, subject, html_body,
              ...
              sent_count, failed_count, sent_at, status, created_at
       FROM campagnes
    ...
    const result = await pool.query(
      `INSERT INTO campagnes
         (name, type, message, subject, html_body, recipients_type,
          activity_id, custom_emails, sent_count, failed_count, status, created_at)
       VALUES ($1,$2,$3,$4,$5,$6,$7,$8,0,0,$9,NOW())
       RETURNING *`,
      [name, type, message, subject, html_body, recipients_type,
       activity_id || null, custom_emails, status]
    );
    ...
    const result = await pool.query(
      `UPDATE campagnes
       SET name=$1, type=$2, message=$3, subject=$4, html_body=$5,
           recipients_type=$6, activity_id=$7, custom_emails=$8, status=$9
       WHERE id=$10
       RETURNING *`,
      [name, type, message, subject, html_body, recipients_type,
       activity_id || null, custom_emails, status, id]
    );
  ...
  try {
    ...
    for (const r of recipients) {
      try {
        ...
        console.error(`[campagne ${id}] échec envoi ${r.email}:`, err.message);
        failed++;
      }
    }
